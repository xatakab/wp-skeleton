...
WP_CHEF_COOKBOOKS_PATH = File.join(File.dirname(__FILE__), 'vendor/miya0001/vccw') # path to the cookbooks (e.g. ~/vccw)
...
Vagrant.configure(2) do |config|
  ...
  config.vm.synced_folder "httpdocs/", "/var/www/wordpress", :create => "true"
  ...
  config.vm.provision :chef_solo do |chef|
    ...
    chef.json = {
      ...
      :vccw => {
        :wordmove => {
          ...
          :wpdir           => File.join('httpdocs', WP_DIR),
          :dbhost          => WP_DB_HOST,
          :dbname          => WP_DB_NAME,
          :dbuser          => WP_DB_USER,
          :dbpassword      => WP_DB_PASS
    ...
    chef.add_recipe "my_wpcli::install"
